...
Publica aerometro_boardings.csv línea por línea al topic:
    aerometro-boardings

Principios:
- No transforma timestamp UTC, tarifa, estación ni ningún valor de negocio.
- Genera `_event_id` determinístico con SHA-256 del archivo + número de fila.
- Usa `_event_id` como key de Kafka.
- Agrega únicamente metadatos técnicos.
- Mantiene todos los registros del archivo fuente.

Pruebas:
    python ingestion/streaming/producer_aerometro.py --dry-run --limit 5
    python ingestion/streaming/producer_aerometro.py --limit 10

Carga completa:
    python ingestion/streaming/producer_aerometro.py
"""
...
RAW_FILE = PROJECT_ROOT / "data" / "raw" / "aerometro_boardings.csv"
...
def build_event(
    ...
    *,
    ...
    return {
        ...
        "_source_name": "aerometro_boardings",
        ...
        producer = Producer(
            {
                ...
                "client.id": "red-metropolitana-aerometro-producer",
                "acks": "all",
