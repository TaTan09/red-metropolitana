#!/usr/bin/env python3
"""
Consumidor Kafka - Aerómetro -> Bronze
...
- NO convierte `timestamp_utc` a hora local en Bronze.
- Deduplica únicamente repeticiones técnicas mediante `_event_id`.
- El estado se guarda en SQLite dentro de data/bronze/_control/.
- Los offsets Kafka se confirman después de persistir el lote.

Prueba:
    python ingestion/streaming/consumer_aerometro.py --max-messages 10

Uso normal:
    python ingestion/streaming/consumer_aerometro.py
"""
...
DEFAULT_TOPIC = "aerometro-boardings"
DEFAULT_GROUP = "bronze-aerometro-v1"
...
def write_batch(
    ...
    if len(partitions) != 1:
        raise RuntimeError(
            "El buffer contiene múltiples particiones; se esperaba una sola."
        )
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    json_path = REPORT_DIR / "streaming_aerometro_consumer.json"
    ...
    lines = [
        ...
        "Bronze conserva el mensaje Kafka completo en `raw_json`.",
        "",
        "El campo `timestamp_utc` permanece en UTC en Bronze. Su conversión "
        "a hora local de Guatemala corresponde a Silver.",
        "",
        "La deduplicación mediante `_event_id` elimina únicamente repeticiones "
        "técnicas del pipeline.",
        "",
    ]

    md_path = REPORT_DIR / "streaming_aerometro_consumer.md"
